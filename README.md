...
```
(node build --copy; node build --composer-install; cd site; vendor/bin/phpunit tests/unit/Espo/Modules/Marketing)
```
...
```
(node build --copy; node build --composer-install; cd site; vendor/bin/phpunit tests/integration/Espo/Modules/Marketing)
```
...
If your extension contains additional PHP packages, you also need to add `site/custom/Espo/Modules/Marketing/vendor` to the *scanDirectories* section in *phpstan.neon* config.
...
* `site/custom/`
* `site/client/custom/`
* `site/tests/unit/Espo/Modules/Marketing`
* `site/tests/integration/Espo/Modules/Marketing`
...
2. Set *bundled* and *jsTranspiled* to true in `src/files/custom/Espo/Modules/Marketing/Resources/module.json`.
3. Add `src/files/custom/Espo/Modules/Marketing/Resources/metadata/app/client.json`
    ```json
    {
        "scriptList": [
            "__APPEND__",
            "client/custom/modules/marketing/lib/init.js"
        ]
    }
    ```
...
Add the library module path to `src/files/custom/Espo/Modules/Marketing/Resources/metadata/app/jsLibs.json`

```json
{
    "some-lib": {
        "path": "client/custom/modules/marketing/lib/some-lib.js"
    }
}
```
